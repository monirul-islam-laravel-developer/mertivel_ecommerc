                            ...
                            <table id="alternative-page-datatable" class="table table-striped dt-responsive nowrap w-100">
                                ...
                                @foreach($sizes as $size)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{route('size.edit', ['id' => $size->id,$size->slug])}}" class="btn btn-success" title="Edit"><i class="uil-edit-alt"></i></a>
                                            <button type="button" onclick="confirmDelete({{$size->id}});" class="btn btn-danger btn-sm" title="Delete">
                                                <i class="ri-chat-delete-fill"></i>
                                            </button>
                                            <form action="{{route('size.delete', ['id' => $size->id])}}" method="POST" id="BrandDeleteForm{{$size->id}}">
                                                @csrf
                                            </form>
                                            <script>
